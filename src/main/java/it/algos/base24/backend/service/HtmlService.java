 ...
 * 1) @Autowired public HtmlService htmlService; <br>
 * <p>
...
public class HtmlService {
    ...
    public Span getSpan(final WrapSpan wrap) {
        ...
        color = wrap.getColor();
        if (color != null) {
            span.getElement().getStyle().set(TypeColor.HTML, color.getTag());
        }

        weight = wrap.getWeight();
        if (weight != null) {
            span.getElement().getStyle().set(FontWeight.HTML, weight.getTag());
        }

        fontHeight = wrap.getFontHeight();
        if (fontHeight != null) {
            span.getElement().getStyle().set(FontSize.HTML, fontHeight.getTag());
        }

        lineHeight = wrap.getLineHeight();
        if (lineHeight != null) {
            span.getElement().getStyle().set(LineHeight.HTML, lineHeight.getTag());
        }

        style = wrap.getStyle();
        if (style != null) {
            span.getElement().getStyle().set(FontStyle.HTML, style.getTag());
        }
    ...
    public Span getSpanVerde(final String message) {
        return getSpan(new WrapSpan(message).color(TypeColor.verde));
    }
    ...
    public Span getSpanBlu(final String message) {
        return getSpan(new WrapSpan(message).color(TypeColor.blue));
    }
    ...
    public Span getSpanRosso(final String message) {
        return getSpan(new WrapSpan(message).color(TypeColor.rosso));
    }
    ...
    public String bold(String stringaIn) {
        ...
        String tagIni = String.format("<span style=\"font-weight:%s\">", FontWeight.bold.getTag());
        String tagEnd = "</span>";
