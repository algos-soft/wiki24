 ...
 * 1) @Autowired public DateService dateService; <br>
 * <p>
...
public class DateService {
                ...
                if (durata < MAX_MINUTI) {
                    ...
                    if (durata < MAX_ORE) {
                        ...
                        if (div < 24) {
                            ...
                            residuo = durata - (div * MAX_MINUTI);
                            if (residuo >= MAX_SECONDI) {
                                tempo += " e " + toText(residuo);
                            }
                        }
                        else {
                            tempo = "1" + GIORNO;
     ...
     * @return la data sotto forma di stringa
     */
    public String get(LocalTime localTime, TypeDate pattern) {
        return localTime.format(DateTimeFormatter.ofPattern(pattern.getPattern(), LOCALE));
    }

    public LocalDateTime now() {
        LocalDateTime dataConNanoSecondi = LocalDateTime.now();
        return LocalDateTime.of(
                dataConNanoSecondi.getYear(),
                dataConNanoSecondi.getMonthValue(),
                dataConNanoSecondi.getDayOfMonth(),
                dataConNanoSecondi.getHour(),
                dataConNanoSecondi.getMinute(),
                dataConNanoSecondi.getSecond()
        );
    }
     ...
     */
    public List<HashMap<String, Object>> getAllGiorni() {
        List<HashMap<String, Object>> listaAnno = new ArrayList<HashMap<String, Object>>();
